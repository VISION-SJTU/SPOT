...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        args:
            ...
            seq_ids - List containing the ids of the videos to be used for training.
                        Note: Only one of 'split' or 'seq_ids' options can be used at the same time.
            data_fraction - Fraction of dataset to be used. The complete dataset is used by default.
        """
        root = env_settings().train_dataset_dirs[self.get_name()] if root is None else root
        super().__init__('GOT-10k', root, image_loader)
    ...
    def get_sequence_info(self, seq_id):
        ...
        result = {'bbox': bbox, 'valid': valid, 'visible': visible, 'visible_ratio': visible_ratio}
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        for i in range(len(frame_ids)):
            ...
            if i == first_showup_index:
                frame_list.append(self._get_frame(seq_path, f))
            else:
