...
def load_pretrained(module, name, checkpoint=None, **kwargs):
    ...
    network_dir = os.path.join(settings.env.var_dir, 'checkpoints', 'ltr', module, name)
    return load_network(network_dir=network_dir, checkpoint=checkpoint, **kwargs)


def load_network(network_dir=None, checkpoint=None, constructor_fun_name=None,
                 constructor_module=None, **kwargs):
    """
...
def _setup_legacy_env():
    ...
    for m in ['local']:
        importlib.import_module('config.' + m)
        sys.modules['dlframework.common.utils.' + m] = sys.modules['config.' + m]
