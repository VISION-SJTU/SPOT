...
def npy_loader(path):
    """ Read image from npy format and returns it in rgb format"""
    try:
        return np.load(path)
    except Exception as e:
        print('ERROR: Could not read numpy format image "{}"'.format(path))
        print(e)
        return None


def npy_writer(path, array):
    try:
        np.save(path, array)
    except Exception as e:
        print('ERROR: Could not write numpy format image "{}"'.format(path))
        print(e)
        return
