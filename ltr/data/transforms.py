...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'avg']
    ...
    def __call__(self, **inputs):
        ...
        for v in inputs.keys():
            if v not in self._valid_all:
                raise ValueError(
                    'Incorrect input \"{}\" to transform. '
                    'Only supports inputs {} and arguments {}.'.format(v, self._valid_inputs, self._valid_args))
...
class TransformBase:
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params):
        ...
        coord_all = torch.tensor([[y1, y1, y2, y2], [x1, x2, x2, x1]], device=bbox.device)
    ...
    def transform_avg(self, avg, *rand_params):
        return avg
...
class RandomVerticalFlip(TransformBase):
    """Vertically flip image randomly with a probability p."""
...
class ToGrayscale(TransformBase):
    ...
    def transform_avg(self, avg, do_grayscale):
        if do_grayscale:
            cvt_res = avg[0] * self.color_weights[0] + \
                      avg[1] * self.color_weights[1] + avg[2] * self.color_weights[2]
            return torch.tensor([cvt_res, cvt_res, cvt_res])
        return avg
...
class RotationEllipse(TransformBase):
    """
    Rotation transform, using Ellipse for bbox conversion (CPU version)
    From ICCV2021 paper: Towards Rotation Invariance in Object Detection
    This Transform Class is implemented by Jilai Zheng, for SPOT strong augmentation
    """
    ...
    def roll(self):
        return random.randint(-self.max_angle, self.max_angle)

    def transform_image(self, image, angle):
        image_avg = image.mean(dim=(1, 2)).tolist()
        return tvisf.rotate(image, angle=angle, fill=image_avg)

    def transform_bbox(self, bbox, image_shape, angle):
        """ input bbox format:[x, y, w, h] """
        h, w = image_shape
        rwrh = 1 / 2 * bbox[2:]
        cxcy = bbox[:2] + rwrh
        a_range = torch.arange(0, 360, 1, device=bbox.device)

        # Ellipse axis for x and y, ranging from angle 0 to 360
        x_axis_ellipse = rwrh[0] * torch.sin(a_range * math.pi / 180) + cxcy[0]
        y_axis_ellipse = rwrh[1] * torch.cos(a_range * math.pi / 180) + cxcy[1]

        # Affine matrix
        matrix = torch.from_numpy(cv.getRotationMatrix2D((w // 2, h // 2),
                                                         angle, 1.0))
        matrix = matrix.to(bbox.device).float()
        axis_xy = torch.stack([x_axis_ellipse, y_axis_ellipse,
                               torch.ones(x_axis_ellipse.shape, device=bbox.device)]).float()

        # Prepare the vector to be transformed
        transformed = torch.matmul(matrix, axis_xy)

        # Ellipse rotation advised by the paper
        trans_max = transformed.max(dim=1).values
        trans_min = transformed.min(dim=1).values

        # Transform back to [x, y, w, h] format
        new_bbox = torch.stack([trans_min[0], trans_min[1], trans_max[0] - trans_min[0],
                                trans_max[1] - trans_min[1]])
